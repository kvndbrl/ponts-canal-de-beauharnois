...
description: Build editorial/magazine/report webpages on a genuine Müller-Brockmann modular grid (International Typographic Style) — not a decorative one. Encodes the discipline (columns + modules + baseline, grotesque type, flush-left, restrained black/white/red palette) and the front-end engineering to make the grid real, visible, and verified: one CSS-variable source of truth, an interactive grid-toggle overlay living in the same content box as the content, subgrid "bands" so every element snaps to a column line, an 8px baseline lock, and runtime optical alignment that puts display type's ink (not its box) on the line. Ships with a scaffold generator (scripts/grid_tokens.py) and a Puppeteer verification harness (scripts/verify_grid.js) that proves 0px adherence. Use when building a magazine spread, grid system, Swiss design, or editorial/longform layout, when asked to "show the grid" or add a grid overlay toggle, or when fixing a grid overlay that's "slapped on top / misaligned" or display type that looks "off the grid" due to glyph side-bearing.
---

> **Script paths:** `scripts/grid_tokens.py` and `scripts/verify_grid.js` are relative to **this skill's directory** (the directory containing this `SKILL.md`). Run them from here, or prefix with the skill's absolute path.
...
- **Real photography.** Ground real subjects in real photos. Host images publicly so they remain reachable wherever the page is published — avoid embedding URLs that only resolve in an authenticated/local context (broken-image trap).
